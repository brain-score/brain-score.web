...
function recalculateColorsForBenchmark(rowData, benchmarkId, hierarchyMap, rootParentCache = null) {
  ...
  rowData.forEach(row => {
    if (row[benchmarkId] && row[benchmarkId].value !== 'X' && row[benchmarkId].value !== null) {
      ...
      const numVal = typeof val === 'string' ? parseFloat(val) : val;
      if (!isNaN(numVal)) {
        const colorCss = calculateRepresentativeColor(numVal, minValue, maxValue, rootParent);
        // Extract rgba value from CSS string (format: "background-color: rgb(...); background-color: rgba(...);")
        // The cell renderer expects just the rgba/rgb value
        const rgbaMatch = colorCss.match(/rgba?\([^)]*\)/);
        if (rgbaMatch) {
          row[benchmarkId].color = rgbaMatch[0];
        } else {
          // Fallback to default if no rgba found
          row[benchmarkId].color = COLOR_NONE;
        }
      }
    }
  });
